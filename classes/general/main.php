...
class RSMonopoly{
	...
	function ShowPanel()
    {
        if ($GLOBALS["USER"]->IsAdmin() && COption::GetOptionString("main", "wizard_solution", "", SITE_ID) == "collected.monopoly")
        {
            $GLOBALS["APPLICATION"]->SetAdditionalCSS("/bitrix/wizards/collected/monopoly/css/panel.css"); 

            $arMenu = Array(
                Array(        
                    "ACTION" => "jsUtils.Redirect([], '".CUtil::JSEscape("/bitrix/admin/wizard_install.php?lang=".LANGUAGE_ID."&wizardSiteID=".SITE_ID."&wizardName=collected:monopoly&".bitrix_sessid_get())."')",
            ...
            $GLOBALS["APPLICATION"]->AddPanelButton(array(
                "HREF" => "/bitrix/admin/wizard_install.php?lang=".LANGUAGE_ID."&wizardName=collected:monopoly&wizardSiteID=".SITE_ID."&".bitrix_sessid_get(),
                "ID" => "monopoly_wizard",
	...
	public static function saveSettings() {
		...
		if(isset($_REQUEST['headType']) && in_array($_REQUEST['headType'],$arHeadType)) {
			COption::SetOptionString('collected.monopoly', 'headType', htmlspecialchars($_REQUEST['headType']) );
		}

		$arHeadStyle = array('style1','style2','style3',);
		if(isset($_REQUEST['headStyle']) && in_array($_REQUEST['headStyle'],$arHeadStyle)) {
			COption::SetOptionString('collected.monopoly', 'headStyle', htmlspecialchars($_REQUEST['headStyle']) );
		}

		$arFilterStyle = array('ftype0','ftype1','ftype2',);
		if(isset($_REQUEST['filterType']) && in_array($_REQUEST['filterType'],$arFilterStyle)) {
			COption::SetOptionString('collected.monopoly', 'filterType', htmlspecialchars($_REQUEST['filterType']) );
		}

		$blackMode = 'N';
		if($_REQUEST['blackMode']=='Y') {
			$blackMode = 'Y';
			COption::SetOptionString('collected.monopoly', 'blackMode', 'Y' );
		}
		COption::SetOptionString('collected.monopoly', 'blackMode', $blackMode );

		if(isset($_REQUEST['gencolor'])) {
			$gencolor = htmlspecialchars($_REQUEST['gencolor']);
			COption::SetOptionString('collected.monopoly', 'gencolor', $gencolor );
		} else {
			$gencolor = COption::GetOptionString('collected.monopoly', 'gencolor', '' );
		}

		$arSidebarPos = array('pos1','pos2',);
		if(isset($_REQUEST['sidebarPos']) && in_array($_REQUEST['sidebarPos'],$arSidebarPos)) {
			COption::SetOptionString('collected.monopoly', 'sidebarPos', htmlspecialchars($_REQUEST['sidebarPos']) );
		}
		...
		if(isset($_REQUEST['optionFrom'])) {
			if($_REQUEST['optionFrom']=='module') {
				COption::SetOptionString('collected.monopoly', 'optionFrom', 'module' );
			} elseif ($_REQUEST['optionFrom']=='session') {
				COption::SetOptionString('collected.monopoly', 'optionFrom', 'session' );
			}
		}
	...
	public static function getSettings($paramName='',$default='') {
		...
		if($paramName!='') {
			...
			if($optionFrom=='session') {
				...
				$return = COption::GetOptionString('collected.monopoly', $paramName, $default);
			}
		}
		return $return;
	}
	...
	public static function generateCssColorFile() {
		$color = COption::GetOptionString('collected.monopoly', 'gencolor', '0084c9');
		$menuTextColor = COption::GetOptionString('collected.monopoly', 'textColorMenu', '');
		$reverse = COption::GetOptionString('collected.monopoly', 'blackMode', 'N');
