...
class RSDevLib {
	public static function Init($arInit) {
		...
		if(in_array("jsfunc",$arInit)) {
			$APPLICATION->AddHeadString("<script>var RSDevLib_BasketEndWord_end1 = \"".GetMessage("RSDF_END_1")."\";var RSDevLib_BasketEndWord_end2 = \"".GetMessage("RSDF_END_2")."\";var RSDevLib_BasketEndWord_end3 = \"".GetMessage("RSDF_END_3")."\";</script>");
			$APPLICATION->AddHeadScript("/bitrix/js/collected.devlibrary/script.js");
		}
	}
	
	public static function GetDataForProductItem(&$arItems,$params=array()) {
		if(\Bitrix\Main\Loader::includeModule('iblock') && is_array($arItems) && count($arItems)>0) {
			// prepare data
			$arElements = array();
			$arElementsIDs = array();
			...
			// add quickbuy
			if(\Bitrix\Main\Loader::includeModule('collected.quickbuy')) {
				$arFilter = array(
					'DATE_FROM' => $iTime,
					'DATE_TO' => $iTime,
					'QUANTITY' => 0,
			...
			// /add quickbuy
			// add da2
			if(\Bitrix\Main\Loader::includeModule('collected.daysarticle2')) {
				$arFilter = array(
					'DATE_FROM' => $iTime,
					'DATE_TO' => $iTime,
					'QUANTITY' => 0,
	...
	public static function GetNoPhoto($arSizes) {
		$return = false;
		$fileid = COption::GetOptionInt('collected.devlibrary', 'no_photo_fileid', 0);
		if($fileid>0) {
