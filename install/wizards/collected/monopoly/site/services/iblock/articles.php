...
if(COption::GetOptionString("collected.monopoly", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "monopoly_articles_".WIZARD_SITE_ID;
		$iblockType = "services";
		...
		{
			WizardServices::ReplaceMacrosRecursive(WIZARD_SITE_PATH."info/", Array("ARTICLES_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}

$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/services/articles-'.LANGUAGE_ID.'.xml';
$iblockCode = "monopoly_articles_".WIZARD_SITE_ID;
$iblockType = "services";
...
//CUserOptions::SetOption("form", "form_element_".$iblockID, array ( 'tabs' => 'edit1--#--'.GetMessage("WZD_OPTION_ARTICLES_1").'--,--ACTIVE--#--'.GetMessage("WZD_OPTION_ARTICLES_2").'--,--ACTIVE_FROM--#--'.GetMessage("WZD_OPTION_ARTICLES_3").'--,--NAME--#--'.GetMessage("WZD_OPTION_ARTICLES_5").'--,--CODE--#--'.GetMessage("WZD_OPTION_ARTICLES_6").'--,--PREVIEW_TEXT--#--'.GetMessage("WZD_OPTION_ARTICLES_8").'--,--DETAIL_TEXT--#--'.GetMessage("WZD_OPTION_ARTICLES_10").'--;--', ));
