...
if(COption::GetOptionString("collected.monopoly", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "monopoly_stocks_".WIZARD_SITE_ID;
		$iblockType = "company";
		...
		{
			WizardServices::ReplaceMacrosRecursive(WIZARD_SITE_PATH."stocks/", Array("STOCKS_IBLOCK_ID" => $iblockID));
			CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."/_index.php", array("STOCKS_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}

$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/company/stocks-'.LANGUAGE_ID.'.xml';
$iblockCode = "monopoly_stocks_".WIZARD_SITE_ID;
$iblockType = "company";
...
//WizardServices::IncludeServiceLang("stocks.php", $lang);
//CUserOptions::SetOption("form", "form_element_".$iblockID, array ( 'tabs' => 'edit1--#--'.GetMessage("WZD_OPTION_STOCKS_1").'--,--ACTIVE--#--'.GetMessage("WZD_OPTION_STOCKS_2").'--,--ACTIVE_FROM--#--'.GetMessage("WZD_OPTION_STOCKS_3").'--,--NAME--#--'.GetMessage("WZD_OPTION_STOCKS_5").'--,--CODE--#--'.GetMessage("WZD_OPTION_STOCKS_6").'--,--PREVIEW_TEXT--#--'.GetMessage("WZD_OPTION_STOCKS_8").'--,--DETAIL_TEXT--#--'.GetMessage("WZD_OPTION_STOCKS_10").'--;--', ));
