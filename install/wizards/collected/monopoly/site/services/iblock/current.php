...
if(COption::GetOptionString("collected.monopoly", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "monopoly_current_".WIZARD_SITE_ID;
		$iblockType = "projects";
		...
		{
			WizardServices::ReplaceMacrosRecursive(WIZARD_SITE_PATH."projects/", Array("CURRENT_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}

$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/projects/current-'.LANGUAGE_ID.'.xml';
$iblockCode = "monopoly_current_".WIZARD_SITE_ID;
$iblockType = "projects";
...
//WizardServices::IncludeServiceLang("current.php", $lang);
//CUserOptions::SetOption("form", "form_element_".$iblockID, array ( 'tabs' => 'edit1--#--'.GetMessage("WZD_OPTION_CURRENT_1").'--,--ACTIVE--#--'.GetMessage("WZD_OPTION_CURRENT_2").'--,--ACTIVE_FROM--#--'.GetMessage("WZD_OPTION_CURRENT_3").'--,--NAME--#--'.GetMessage("WZD_OPTION_CURRENT_5").'--,--CODE--#--'.GetMessage("WZD_OPTION_CURRENT_6").'--,--PREVIEW_TEXT--#--'.GetMessage("WZD_OPTION_CURRENT_8").'--,--DETAIL_TEXT--#--'.GetMessage("WZD_OPTION_CURRENT_10").'--;--', ));
