...
if(COption::GetOptionString("collected.monopoly", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "monopoly_history_".WIZARD_SITE_ID;
		$iblockType = "company";
		...
		{
			WizardServices::ReplaceMacrosRecursive(WIZARD_SITE_PATH."about/", Array("HISTORY_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}

$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/company/history-'.LANGUAGE_ID.'.xml';
$iblockCode = "monopoly_history_".WIZARD_SITE_ID;
$iblockType = "company";
...
//WizardServices::IncludeServiceLang("history.php", $lang);
//CUserOptions::SetOption("form", "form_element_".$iblockID, array ( 'tabs' => 'edit1--#--'.GetMessage("WZD_OPTION_HISTORY_1").'--,--ACTIVE--#--'.GetMessage("WZD_OPTION_HISTORY_2").'--,--ACTIVE_FROM--#--'.GetMessage("WZD_OPTION_HISTORY_3").'--,--NAME--#--'.GetMessage("WZD_OPTION_HISTORY_5").'--,--CODE--#--'.GetMessage("WZD_OPTION_HISTORY_6").'--,--PREVIEW_TEXT--#--'.GetMessage("WZD_OPTION_HISTORY_8").'--,--DETAIL_TEXT--#--'.GetMessage("WZD_OPTION_HISTORY_10").'--;--', ));
