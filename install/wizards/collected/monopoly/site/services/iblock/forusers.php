...
if(COption::GetOptionString("collected.monopoly", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "monopoly_forusers_".WIZARD_SITE_ID;
		$iblockType = "docs";
		...
		{
			WizardServices::ReplaceMacrosRecursive(WIZARD_SITE_PATH."about/", Array("FORUSERS_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}

$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/docs/forusers-'.LANGUAGE_ID.'.xml';
$iblockCode = "monopoly_forusers_".WIZARD_SITE_ID;
$iblockType = "docs";
...
//WizardServices::IncludeServiceLang("forusers.php", $lang);
//CUserOptions::SetOption("form", "form_element_".$iblockID, array ( 'tabs' => 'edit1--#--'.GetMessage("WZD_OPTION_FORUSERS_1").'--,--ACTIVE--#--'.GetMessage("WZD_OPTION_FORUSERS_2").'--,--ACTIVE_FROM--#--'.GetMessage("WZD_OPTION_FORUSERS_3").'--,--NAME--#--'.GetMessage("WZD_OPTION_FORUSERS_5").'--,--CODE--#--'.GetMessage("WZD_OPTION_FORUSERS_6").'--,--PREVIEW_TEXT--#--'.GetMessage("WZD_OPTION_FORUSERS_8").'--,--DETAIL_TEXT--#--'.GetMessage("WZD_OPTION_FORUSERS_10").'--;--', ));
