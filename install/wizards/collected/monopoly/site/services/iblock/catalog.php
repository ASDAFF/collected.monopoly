...
if(COption::GetOptionString("collected.monopoly", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "monopoly_catalog_".WIZARD_SITE_ID;
		...
		{
			WizardServices::ReplaceMacrosRecursive(WIZARD_SITE_PATH."about/", Array("CATALOG_IBLOCK_ID" => $iblockID));
			WizardServices::ReplaceMacrosRecursive(WIZARD_SITE_PATH."catalog/", Array("CATALOG_IBLOCK_ID" => $iblockID));
...
$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/catalog/catalog-'.LANGUAGE_ID.'.xml';
$iblockCode = "monopoly_catalog_".WIZARD_SITE_ID;
$iblockType = "presscenter";
