...
if(COption::GetOptionString("collected.monopoly", "wizard_installed", "N", WIZARD_SITE_ID) == "Y" && !WIZARD_INSTALL_DEMO_DATA){
	if($wizard->GetVar('rewriteIndex', true) && $wizard->GetVar('siteLogoSet', true)){
		$iblockCode = "monopoly_salesdepartment_".WIZARD_SITE_ID;
		$iblockType = "company";
		...
		{
			WizardServices::ReplaceMacrosRecursive(WIZARD_SITE_PATH."about/", Array("SALESDEPARTMENT_IBLOCK_ID" => $iblockID));
		}
	}
	return;
}

$iblockXMLFile = WIZARD_SERVICE_RELATIVE_PATH.'/xml/company/salesdepartment-'.LANGUAGE_ID.'.xml';
$iblockCode = "monopoly_salesdepartment_".WIZARD_SITE_ID;
$iblockType = "company";
...
//CUserOptions::SetOption("form", "form_element_".$iblockID, array ( 'tabs' => 'edit1--#--'.GetMessage("WZD_OPTION_SALESDEPARTMENT_1").'--,--ACTIVE--#--'.GetMessage("WZD_OPTION_SALESDEPARTMENT_2").'--,--ACTIVE_FROM--#--'.GetMessage("WZD_OPTION_SALESDEPARTMENT_3").'--,--NAME--#--'.GetMessage("WZD_OPTION_SALESDEPARTMENT_5").'--,--CODE--#--'.GetMessage("WZD_OPTION_SALESDEPARTMENT_6").'--,--PREVIEW_TEXT--#--'.GetMessage("WZD_OPTION_SALESDEPARTMENT_8").'--,--DETAIL_TEXT--#--'.GetMessage("WZD_OPTION_SALESDEPARTMENT_10").'--;--', ));
